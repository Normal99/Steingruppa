...
async function submitRequest() {
  ...
  let requestData = { type: requestType, message, stoneDocId, timestamp: new Date().toISOString() };

  if (requestType === "add") {
    requestData.details = {
      kasse: DOMPurify.sanitize(document.getElementById('req-add-kasse').value.trim()),
      steingruppe: DOMPurify.sanitize(document.getElementById('req-add-steingruppe').value.trim()),
      id: DOMPurify.sanitize(document.getElementById('req-add-id').value.trim()),
      sted: DOMPurify.sanitize(document.getElementById('req-add-sted').value.trim())
    };
  } else if (requestType === "update") {
    requestData.details = {
      current: {
  ...
  try {
    ...
    toggleRequestFields();
    closeRequestModal(); // Close the modal after submission
  } catch (error) {
